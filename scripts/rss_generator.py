...
RSS_FILE = config["outName"]
SITE_TITLE = config["title"]
SITE_URL = config["url"]
SITE_DESC = config["desc"]
...
def generate_rss_items(doc, sources):
    ...
    for src in sources:
        ...
        title.appendChild(doc.createTextNode(src["title"]))

        link = doc.createElement("link")
        l = urljoin(SITE_URL, src["finalPath"])
        link.appendChild(doc.createTextNode(l))

        desc = doc.createElement("description")
        desc.appendChild(doc.createTextNode(src["summary"]))

        pubDate = doc.createElement("pubDate")
        date = rss_pubdate_from_date(datetime.fromisoformat(src["date"]))
...
def generate_rss(sources):
    ...
    atomLink.setAttribute("href", urljoin(SITE_URL, RSS_FILE))
